...
1. Create a Guru and connect a model. The new Guru already has credential-free data tools; add an SEC contact email from Chat if you want filings from the regulator.
...
5. The user can revert an applied Wiki or Lens write. Evidence and Decisions can be saved as experience even when `Update memory` is off. The original Decision is not rewritten.
6. Charts stay as the current Chat artifact. Restart keeps Memory; disposable run output does not become a second knowledge store.
